/* 
 ...
 Thanks to sereneinserenade and all the contributors on GitHub for this plugin.
...
export const Video = Node.create({
	...
	addCommands() {
		return {
			setVideo:
				(src: string, title?: string, controls?: boolean) =>
				({ commands }) =>
					commands.insertContent(
						`<video controls="${controls ?? true}" style="width: 100%" src="${src}" ${
							!!title ? `title="${title}"` : ""
						} />`
					),
